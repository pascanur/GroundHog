...
def comp_scores(s_word, t_word, model, max_phrase_length, batch_size):
    ...
    for trgt in trgt_p_i:
       for src in chunks(src_p_i, batch_size):
           i, j = trgt[1]
           trgt_phrase = numpy.tile(trgt[0], (len(src), 1))
           trgt_mask = numpy.ones((trgt_phrase.shape[0], trgt_phrase.shape[1]), dtype="float32")

           m = max([len(a[0]) for a in src])
           ...
           # Substitute?
           can_scores = scorer(src_phrase.T, trgt_phrase.T, src_mask.T, trgt_mask.T)

           for idx, el in enumerate(src):
               print i, j, el[1][0], el[1][1], can_scores[0][idx]
            
           for index, pairing in enumerate(can_scores[0]):
               if pairing < scores[i,j]:
                   scores[i,j] = pairing
                   segment[i,j] = src[index][1]
           
           #print i,j, segment, can_scores
...
def find_max(source, target, model, max_phrase_length, n_phrase_chosen_per_s):
    scores, phrases = comp_scores(source, target, model, max_phrase_length)
    best_scores = numpy.sort(scores.flatten())[0:n_phrase_chosen_per_s]
    best_scores_idx = [[i,j] for [i,j] in product(xrange(scores.shape[0]), xrange(scores.shape[1]))\
                         if scores[i,j] in best_scores] 

    phrase_table = []
    for key in best_scores_idx:
        i, j = key
        k, l = phrases[i, j]
        source_phrase = " ".join(source.strip().lower().split()[k:l+1])
        target_phrase = " ".join(target.strip().lower().split()[i:j+1])
        phrase_table.append([source_phrase, target_phrase, scores[i,j]])
...
if 0:
    ...
    max_text_len = 1
    max_phrase_length = 5
    n_phrase_chosen_per_sentence = 5
    s_text = []
    t_text = []
    phrase_table = []
    with open("/data/lisatmp3/pougetj/dev08_11.en") as f:
    ...
    with open("/data/lisatmp3/pougetj/dev08_11.fr") as g:
        for i, line in enumerate(g):
            ...
            t_text.append(line)
    for source, target in izip(s_text, t_text):
        phrase_table.append(find_max(source, target, model, max_phrase_length, n_phrase_chosen_per_sentence))
...
if 0:
    for src in src_p_i:
       for trgt in chunks(trgt_p_i, batch_size):
           i, j = src[1]
           src_phrase = numpy.tile(src[0], (len(trgt), 1))
           src_mask = numpy.ones((src_phrase.shape[0], src_phrase.shape[1]), dtype="float32")

           m = max([len(a[0]) for a in trgt])
           trgt_phrase = numpy.empty((m,))
           trgt_mask = numpy.empty((m,))

           for p in trgt:
               padded_p = numpy.pad(p[0], (0, m-len(p[0])), mode='constant', \
                                    constant_values=(0, eol_trgt)).astype("int64")
               trgt_phrase = numpy.vstack((trgt_phrase, padded_p)) 
               padded_mask = numpy.pad(numpy.ones(len(p[0]), dtype="float32"), (0, m-len(p[0])), \
                                    mode="constant", constant_values=(0, 0)).astype("int64")
               trgt_mask = numpy.vstack((trgt_mask, padded_mask))
