...
def get_data(state, rng):
    ...
    train_data = TMIteratorPytables(
        ...
        use_infinite_loop=state['infinite_loop'],
        can_fit=False,
        queue_size=10,
...
def do_experiment(state, channel):
    ...
    try:
        ...
        if state['encode']:
            savefile = open(state['encode_file'], 'w')
            encode_fn = theano.function(
                    inputs=[enc_dec.x, enc_dec.x_mask],
                    outputs=[enc_dec.training_c.out])
            ...
            for batch in data_iter:
                ...
                [code] = encode_fn(batch['x'], batch['x_mask'])
                for s in xrange(code.shape[0]):
                    print >>savefile, ' '.join(map(str,code[s,:]))
                ...
                if count % 10000 == 0:
                    ...
                    print '.',
                print count, n_samples, up_time/code.shape[1]
            savefile.close()
...
def prototype_state():
    ...
    # Output all the representations
    state['encode'] = False
